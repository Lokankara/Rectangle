...
//        System.out.println(names);
        Supplier<TreeSet<String>> treeSetSupplier = () -> new TreeSet<>();
...
//        List<Integer> integerList = list.stream().filter(i -> i > list.stream().mapToInt(integer -> integer).sum()/ list.size()).toList();
