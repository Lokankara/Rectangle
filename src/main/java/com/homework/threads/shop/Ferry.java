package com.homework.threads.shop;
...
public class Ferry {
    ...
    private Ferry() {
        ...
        if (area == null || bearingCapacity == null) {
            ...
            System.out.println("Created ferry with default parameters area = " + area + " bearing capacity = " + bearingCapacity);
        }
    ...
    public static void ferryConfiguration(int ferryArea, int ferryBearingCapacity) {
        System.out.println("Create ferry with parameters area = " + ferryArea + " bearing capacity = " + ferryBearingCapacity);
    ...
    public boolean loadVehicle(Vehicle vehicle) {
        lock.lock();
        System.out.println("Try load: " + Thread.currentThread().getName());
    ...
    public boolean runToUnload() {
        ...
        try {
            if (!isLoading.get()) {
                ...
                freeBearingCapacity = new AtomicInteger(bearingCapacity.get());
                result = true;
                System.out.println("Ferry is unload");
            }
        } finally {
            lock.unlock();
        }
